...
func bootstrapVideo(mode, mcpPath, outOverride, addr string, maxConcurrency int, password string, forceAudio bool) (*runtime, int) {
	...
	points, err := server.NewPointsStore(discussions)
	if err != nil {
		fmt.Fprintln(os.Stderr, "points db:", err)
		cancel()
		return nil, 1
	}
	...
	rt.srv = server.New(server.Deps{
		...
		VoiceCatalog:          server.NewVoiceCatalogStore(env.RedisURL, log),
		Entitlements:          server.NewEntitlementsStore(env.RedisURL, log),
		Log:                   log,
		...
		ServiceToken:          env.DashboardServiceToken,
		AuthIssuer:            env.AuthIssuer,
		AdminAllowedClientIDs: env.AdminAllowedClientIDs,
