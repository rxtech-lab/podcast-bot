...
type Card struct {
	...
	AgentDocument *AgentDocumentInfo      `json:"agent_document,omitempty"`
}
...
// AgentDocumentInfo is the content-free payload for a persistent Markdown
// document written by the chat agent.
type AgentDocumentInfo struct {
	ID           string `json:"id"`
	Title        string `json:"title"`
...
type Retriever interface {
	...
	CreateAgentDocument(ctx context.Context, discussionID, conversationID, toolCallID, title, markdown string) (*AgentDocumentInfo, error)
}
...
- write_document: write a persistent Markdown document for the user and show it as a document card.
...
- When the user asks you to write, draft, or create a document, call write_document as the terminal action. Write complete, polished Markdown. Add a fenced Mermaid code block (language: mermaid) when a diagram materially clarifies a process, relationship, hierarchy, or sequence; use valid Mermaid syntax and omit it when prose is clearer.
...
- write_document: write a persistent Markdown document and show it as a document card. It may optionally be linked to one podcast.
...
- When the user asks you to write, draft, or create a document, call write_document as the terminal action. Write complete, polished Markdown. Add a fenced Mermaid code block (language: mermaid) when a diagram materially clarifies a process, relationship, hierarchy, or sequence; use valid Mermaid syntax and omit it when prose is clearer. Set discussion_id only when the document belongs to exactly one verified podcast; documents covering multiple podcasts remain global.
- Keep answers conversational and concise. Reply in the requested language.`
...
func isBatchPresentationTool(name string) bool {
	switch name {
	case "display_podcasts", "show_podcasts", "show_highlight_lines", "display_mindmap", "display_ppt", "write_document":
		return true
	default:
		return false
	}
...
func (s *session) dispatch(ctx context.Context, tc llm.ToolCall) (output string, card *Card, isErr bool) {
	...
	case "write_document":
		...
		if err := json.Unmarshal([]byte(tc.Arguments), &args); err != nil {
			return "write_document requires valid JSON", nil, true
		}
		args.Title = strings.TrimSpace(args.Title)
		args.Markdown = strings.TrimSpace(args.Markdown)
		if args.Title == "" || args.Markdown == "" {
			return "title and markdown are required", nil, true
		}
		if len(args.Title) > 200 || len(args.Markdown) > 200_000 {
			return "document is too large", nil, true
		}
		discussionID := s.opts.DiscussionID
		if s.opts.Scope == ScopeGlobal {
			discussionID = strings.TrimSpace(args.DiscussionID)
		}
		document, err := s.retriever.CreateAgentDocument(ctx, discussionID,
			s.opts.ConversationID, tc.ID, args.Title, args.Markdown)
		if err != nil {
			return "write_document failed: " + err.Error(), nil, true
		}
		return "Document saved and shown to the user. Do not add assistant prose in this turn.",
			&Card{Kind: CardDocument, AgentDocument: document}, false
	default:
