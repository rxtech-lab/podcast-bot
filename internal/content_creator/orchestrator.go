...
type Orchestrator struct {
	...
	// the live-captions endpoint can read cues-so-far (p.vtt grows as turns
	// are spoken). Guarded by liveMu; nil before Run starts and after it
	// returns (at which point subtitleCues holds the final snapshot).
	liveMu   sync.RWMutex
	livePipe *Pipeline
...
func New(env *config.Env, topic *config.DebateTopic, mcpCfg *config.MCPConfig,
	...
	pricedClient := func(baseURL, key, model string) *llm.Client {
		return llm.New(baseURL, key, model).
			WithUsageRecorder(recordUsage).
			WithPricing(env.LLMInputCostPerMillion, env.LLMOutputCostPerMillion)
	}
...
func (o *Orchestrator) Run(ctx context.Context) error {
	...
	o.liveMu.Unlock()
	o.forceMu.Lock()
	forceStopNow := o.forceStopRequested
	o.forceMu.Unlock()
	if forceStopNow {
		pipe.ForceStop()
	}
...
// ForceStop asks the active pipeline to stop producing new turns immediately.
// The video job runner still finalizes and uploads whatever audio/video has
...
// LiveSubtitleCues returns the cues accumulated so far. While Run is in flight
// it reads the running pipeline (cues grow as turns are spoken); once Run has
// returned it falls back to the final snapshot. This backs the live-captions
// endpoint so a client can poll for captions during generation.
func (o *Orchestrator) LiveSubtitleCues() []SubtitleCue {
	o.liveMu.RLock()
	pipe := o.livePipe
	o.liveMu.RUnlock()
	if pipe != nil {
		return pipe.SubtitleCues()
	}
	return o.SubtitleCues()
}
